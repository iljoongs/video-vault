...
- **배우 관리 화면(`ActorManagerWindow`, 기본 크기 1000x1000 — 마지막으로 조절한 크기가 있으면 그 크기로 열림, 아래 "창 전체 크기도 기억한다" 참고)**: 메인창 "도구 > 배우 관리" 메뉴 또는 관리 리스트 툴바의 "배우 관리" 버튼(태그 관리 버튼 옆)으로 연다.
  ...
  - **목록 아래 입력줄: 텍스트박스 하나 + "추가"/"변경"/"삭제" 버튼(이 순서)**(2026-08-27 재설계, **구현 완료**, 사용자 요청) — 예전에는 "추가"용 텍스트박스(`NewActorBox`)와 "이름 변경"용 텍스트박스(`RenameBox`)가 줄을 나눠 따로 있고 버튼 라벨도 "선택 배우 이름 변경"/"선택 배우 삭제"로 길었으며 맨 아래 "닫기" 버튼도 있었는데, 하나의 텍스트박스(`ActorBox`)로 합치고 버튼 3개(추가/변경/삭제)만 남기는 것으로 정리됐다(태그 관리 창의 같은 날 재설계와 동일한 패턴, [태그 관리](tag-management.md) 참고). **"닫기" 버튼은 삭제되었다** — 제목표시줄 X, Alt+F4로 여전히 닫을 수 있다.
  - **추가**: `ActorBox`에 새 배우 이름을 입력하고 "추가"를 누르면 목록에 추가된다(목록 선택 여부와 무관, 동일성 판정은 태그와 동일하게 앞뒤 공백 제거 + 대소문자 무시). **추가된 배우를 목록에서 바로 선택하고 그 위치로 스크롤한다**(2026-08-02 추가, **구현 완료**) — 태그 관리와 동일한 패턴(`ActorsListBox.SelectedItem`/`ScrollIntoView`), 이름 기준 정렬이라 새 배우가 임의 위치에 삽입되므로 직접 찾지 않아도 바로 보이게 한다.
  - **변경(이름 변경)**: 목록에서 바꿀 배우를 먼저 선택하고, `ActorBox`에 새 이름을 입력한 뒤 "변경"을 누른다(`ActorBox`는 자동으로 채워지지 않는다 — 추가와 같은 텍스트박스를 공유할 뿐, 선택한 배우의 현재 이름을 미리 채워주지는 않는다). 이름을 바꾸면 이 배우가 지정된 모든 관리 리스트 항목의 `Actors`에도 새 이름이 반영된다(참조 무결성 유지, 태그 이름 변경과 동일한 패턴). 썸네일 파일이 있으면 새 이름 기준 파일명으로 함께 rename을 시도한다(부가 정리, 실패해도 이름 변경 자체는 유지 — `RenameHelper`의 썸네일 rename과 동일한 원칙). 이름 변경 후 정렬 위치 갱신을 위해 `_actorsView.Refresh()`를 호출한다.
  - **삭제**: 목록에서 배우를 선택하고 "삭제"를 누르면 마스터 목록에서 삭제되고, 이 배우를 사용 중인 관리 리스트 항목들의 `Actors`에서도 제거되며, 배우의 썸네일 파일도 함께 삭제를 시도한다(부가 정리, 실패해도 무시). `ActorBox` 내용과는 무관하다.
  - **썸네일 지정/삭제**: 창 오른쪽에 선택된 배우의 썸네일 뷰어가 있고, 그 위에 "썸네일 추가"/"썸네일 삭제" 버튼이 나란히 있다. **뷰어는 사진의 원본 비율·크기 그대로, 테두리가 여백 없이 딱 맞게 감싼다**(2026-08-27 최종 수정, 사용자 피드백에 따라 두 단계로 조정) — ① 처음엔 고정 200x150 박스에 `Stretch="UniformToFill"`이라 가로로 긴 박스가 세로로 긴 인물 사진의 위/아래를 잘라냈고, ② `Stretch="Uniform"`(비율 유지, 안 잘림)으로 바꿨지만 여전히 고정 박스 크기라 사진과 박스의 비율이 다르면 위아래/좌우로 빈 여백(레터박스)이 남았다. 최종적으로 `Border`의 `Width`/`Height`를 아예 없애고(`MinWidth="100" MinHeight="100"`만 남김, 썸네일이 없을 때 드롭 영역이 너무 작아지지 않게) `Image`도 `Stretch="None"`으로 바꿔, 저장된 썸네일 파일의 실제 픽셀 크기·비율 그대로 렌더링되고 `Border`는 그 크기에 맞춰 자동으로 딱 맞게(auto-size) 감싼다 — 레터박스도 크롭도 없다. 참고로 배우 썸네일은 [공통 관리](common-management.md)/`ThumbnailHelper.CreateActorThumbnail`이 저장 시점에 100x100 이내로 리사이즈하고 원본은 보관하지 않으므로, 여기서 말하는 "원본 크기"는 그 리사이즈된 파일 자체의 크기다(진짜 촬영 원본 해상도가 아님). 추가는 파일 선택 대화상자 또는 드래그 앤 드롭(`DragDropImageHelper` 재사용, 인터넷 이미지 드래그도 동일하게 지원 — [공통 관리](common-management.md) 참고)으로, 삭제는 썸네일 파일을 디스크에서 지우고 `ThumbnailPath`를 null로 되돌린다(즉시 반영, 썸네일이 없으면 안내만 표시). 동영상 썸네일 뷰어와 달리 **원본을 클릭해서 크게 보는 기능은 없다** (원본 자체를 보관하지 않으므로).
  - **오른쪽 패널 크기 조절**(2026-08-27 추가, **구현 완료**, 사용자 요청): 왼쪽 배우 그리드와 오른쪽 패널(썸네일/배우 정보/Credits) 사이에 세로로 긴 `GridSplitter`가 있어, 좌우로 드래그하면 오른쪽 패널 전체 가로 폭이 조절된다(150~600px, `RightPanelColumn`). **썸네일 뷰어 자체의 크기는 이 폭과 무관하다** — 처음엔 `HorizontalAlignment="Stretch"`로 패널 폭에 맞춰 썸네일도 함께 늘어나게 했으나(패널을 늘렸다 줄였다 할 때마다 썸네일 크기까지 덩달아 바뀌는 게 산만하다는 피드백으로 되돌림), 이후 원본 비율/크기 그대로 보여주도록 다시 바뀌었다(위 "썸네일 지정/삭제" 참고) — 결과적으로 패널이 넓어지면 썸네일 오른쪽에 남는 공간이 그냥 빈 여백으로 남는다. **마지막 패널 폭은 창을 닫을 때 기억해뒀다가 다시 열면 그대로 복원된다** — `ActorManagerWindow.RememberedRightPanelWidth`(정적 프로퍼티, 앱 실행 중 메모리에 보관)를 `MainWindow`가 시작 시 `AppSettings.ActorManagerRightPanelWidth`에서 불러와 채우고 종료 시 다시 그 값을 저장한다(`WindowPositionMemory`와 같은 패턴이나 창이 하나뿐이라 Dictionary 없이 정적 프로퍼티 하나로 충분). 세로 크기는 이 패널 자체가 아니라 아래 품번 목록만 조절 대상이다(다음 항목 참고). **오른쪽 패널 좌우 여백은 서로 같다**(2026-08-27 추가, 사용자 요청) — 바깥 `Grid`가 `Margin="10"`(창 테두리 기준 사방 10)이라 오른쪽 패널 오른쪽 끝은 창 오른쪽 테두리에서 10만큼 떨어져 있는데, 왼쪽 끝(스플리터와 맞닿는 쪽)은 원래 여백이 0이라 비대칭이었다. 오른쪽 패널 `Grid`에 `Margin="10,0,0,0"`을 줘서 왼쪽도 10으로 맞췄다(창 크기 조절바인 `GridSplitter` 자체는 이 여백 대상이 아니다 — 스플리터와 패널 사이에만 이 여백이 들어간다).
  - **창 전체 크기도 기억한다**(2026-08-27 추가, **구현 완료**, 사용자 요청) — 위치(Left/Top)는 `WindowPositionMemory`로 기억하고, 크기(Width/Height)는 공용 `WindowSizeMemory`(2026-08-29부터 [속성 관리](properties-management.md)/[태그 관리](tag-management.md)/[시리즈 관리](series-management.md) 3개 창과 공유하는 저장소로 일반화됨 — 이전에는 `ActorManagerWindow` 전용 정적 프로퍼티였다)로 기억한다. 생성자에서 `WindowSizeMemory.TryGetSize(nameof(ActorManagerWindow), ...)`로 값이 있으면 `Width`/`Height`에 적용하고, `Closed` 시점에 `WindowSizeMemory.Remember(nameof(ActorManagerWindow), WindowState == Normal ? Width/Height : RestoreBounds.Width/Height)`(최대화 상태로 닫았다면 최대화 이전 "정상" 크기 — `MainWindow` 자신의 크기 기억 로직과 동일한 원칙)로 저장한다. `MainWindow`가 시작 시 `AppSettings.WindowSizes`에서 불러와 채우고 종료 시 다시 저장한다. 같은 세션 내 재오픈과 앱 완전 재시작(`settings.json` 왕복) 둘 다 확인함 → [공통 관리](common-management.md)의 "창 관리 정책" 규칙 4 참고.
  ...
  - **우클릭 → "배우 정보 수정"**: 목록의 배우를 우클릭하면 `ActorItemContextMenu`(다른 우클릭 메뉴들과 동일하게 `Window.Resources`에 선언 후 `ItemContainerStyle`에서 `StaticResource`로 참조하는 패턴)가 뜨고, "배우 정보 수정"을 클릭하면 `ActorInfoWindow`가 열린다. 이 창에서 이름/출생년도/키/신체정보를 편집할 수 있다(출생년도/키/신체정보는 비워두면 삭제됨). 이름을 바꾸면 대화상자 자체는 다른 배우와의 이름 중복만 검사하고, 실제 rename 동기화(썸네일 파일 rename + 관리 리스트 항목들의 `Actors` 참조 갱신)는 "변경" 버튼과 공유하는 `RenameActorAndSync`가 처리한다(중복 로직 방지) — **구현 완료**.
  - **품번(이 배우가 출연한 작품 목록 = `ActorItem.Credits`, 2026-08-02 추가, 구현 완료; 라벨은 2026-08-27 "Credits"→"품번"으로 변경, 사용자 요청)**: 썸네일 정보/배우 정보 밑에 **"품번"** 라벨과 그 옆(오른쪽 정렬) **"적용"/"추가"** 버튼(이 순서)이 있고, 그 아래 이 배우의 `ActorItem.Credits`(품번 문자열 목록)를 태그 칩과 같은 모양(둥근 배경, 패딩)의 칩으로 나열한다. **품번 기준 오름차순으로 정렬되어 표시된다**(2026-08-09 추가, `OrderBy(code, StringComparer.OrdinalIgnoreCase)`) — `Credits`(저장 순서)는 그대로 두고 화면 표시만 정렬한다. **"추가" 버튼은 2026-08-31 "작품 추가"에서 이름이 짧아졌다**(사용자 요청, 태그/시리즈 관리 창의 같은 버튼과 이름 통일 — 여는 대화상자(`AddCreditWindow`)의 창 제목 "작품 추가"는 그대로다).
    ...
    - **드래그 앤 드롭으로 품번 추가**(2026-08-15 추가, **구현 완료**): "작품 추가" 대화상자를 열지 않고도, 브라우저 등에서 선택한 텍스트를 Credits 칩 목록(`CreditsList`) 영역으로 바로 끌어다 놓으면 그 텍스트가 품번으로 즉시 추가된다. `AddCreditWindow`와 동일한 정규화 규칙(소문자, 앞뒤 공백 제거)을 따르고, 같은 `AddCreditToActor`(시리즈 관리 창은 `AddCreditToSeries`)를 재사용해 중복 검사·관리 리스트 자동 태깅·화면 갱신이 "작품 추가" 버튼 경로와 완전히 동일하게 적용된다.
      - **드롭 영역/세로 크기(2026-08-22, 2026-08-27 변경, 구현 완료)**: 처음에는 `MinHeight="30"`(내용에 딱 맞게 줄어듦) → `Height="300"` 고정(시리즈 관리 창처럼 눈에 띄는 고정 영역)을 거쳐, 최종적으로 **시리즈 관리 창과 동일하게 `*` 행에 넣어 남는 세로 공간을 전부 채우도록** 바뀌었다(2026-08-27, 사용자 요청 — "작품 리스트를 현재 창 기준으로 세로 크기를 최대로"). 오른쪽 패널 전체를 `StackPanel`에서 2행짜리 `Grid`(`RowDefinition Height="Auto"`+`Height="*"`)로 바꿔, 썸네일/배우 정보/Credits 헤더는 Row 0(내용에 맞는 높이)에, `CreditsList`를 감싼 `ScrollViewer`는 Row 1(`*`, 창 크기에 따라 자동으로 남는 공간 전부)에 배치했다 — 예전엔 오른쪽 패널이 고정폭 `StackPanel`이라 `*` 행 자체를 쓸 수 없었지만, `Grid`로 바꾸면서 가능해졌다. **테두리는 2026-08-27 추가로 다시 생겼다**(사용자 요청, 시리즈 관리 창도 동일) — `ScrollViewer`를 회색 `CornerRadius="8"` 둥근 `Border`(배경 없음)로 한 번 더 감쌌고, `ScrollViewer`에 `Padding="6"`도 줘서 칩이 모서리에 바로 닿지 않게 했다(태그 관리 창의 품번 영역과 동일한 스타일 — [태그 관리](tag-management.md) 참고). 이벤트 핸들러(`CreditsList_DragOver`/`CreditsList_Drop`)는 `sender`를 쓰지 않고 `e.Data`만 참조하므로 호스트를 옮겨도 로직 변경이 필요 없었다.
    - **"적용" 버튼**(2026-08-31 추가, "추가" 버튼 왼쪽, 사용자 요청): 선택한 배우의 Credits 중 관리 리스트에 아직 매칭되는 항목이 없는(연한 색 칩) 품번을 일괄로 [동영상 파일 관리](video-file-management.md)의 "파일 없이 추가"와 동일한 신규(placeholder, `IsPlaceholder=true`/`IsValid=false`/`IsExist=false`) 항목으로 등록한다(`ApplyMissingCredits_Click`, [태그 관리](tag-management.md)/[시리즈 관리](series-management.md)의 같은 이름 버튼과 동일한 패턴). 이미 매칭되는 항목이 있는 품번(진한 색 칩)은 건드리지 않는다. 새로 만든 항목은 이 배우가 출연했음을 이미 알고 있으므로 `Actors`에도 이 배우를 채운다(`SetActors(new List<string> { actor.Name })`). 결과는 상태바에 "N개 품번을 신규 파일로 등록했습니다."(등록할 대상이 없으면 정보 메시지)로 표시되며, 확인 대화상자 없이 바로 실행된다(기존 값을 덮어쓰지 않는 안전한 연산이므로). 이렇게 등록된 항목은 메인 창의 [빠른 보기 패널](video-file-management.md) "신규 파일" 체크박스로 모아 볼 수 있다.

## 작품 추가 창 (`AddCreditWindow`)

(2026-08-02 추가, 2026-08-09 연속 추가 방식으로 개편, 구현 완료): "작품 추가" 버튼(배우 관리 창의 라벨 — 시리즈 관리 창에서는 같은 역할의 버튼이 2026-08-27부터 "추가"로 이름이 짧아졌다, [시리즈 관리](series-management.md) 참고)으로 여는 대화상자(기본 크기 520x420). **배우 관리/시리즈 관리 창이 공유하는 완전히 동일한 대화상자다** — 시리즈 관리 창(`SeriesManagerWindow`)에서도 같은 창을 그대로 재사용한다.
...
- `OnFileDeleted(item, masterActors)`(2026-08-16 추가, 구현 완료; 2026-09-13 호출 순서 수정): [동영상 파일 관리](video-file-management.md)의 "완전삭제"(`MainWindow.PermanentlyDeleteManaged_Click`)에서 호출한다. 이 파일에 태깅되어 있던 모든 배우에 대해 내부적으로 `OnActorRemovedFromItem`을 호출해서, 각 배우의 Credits에서 이 파일의 품번을 제거한다 — 완전삭제는 항목 자체가 사라지므로, 정리하지 않으면 그 품번이 실제로는 존재하지 않는 파일인 채로 "파일 없음"(연한 색) 상태로 배우 관리 창에 고아처럼 남는다. **"제거"(소프트 삭제, `IsValid = false`, 복구 가능)는 대상이 아니다** — 항목이 관리 리스트에 그대로 남아있어 Credits와 어긋나지 않기 때문이다. **호출 순서 버그(2026-09-13 발견·수정, [태그 관리](tag-management.md)의 `TagCreditSync` 도입 중 실제로 재현됨)** — 예전에는 `_managedItems.Remove(item)`보다 **먼저** 이 메서드를 호출했는데, 배우 관리 창이 열려 있고 마침 그 배우가 선택되어 있으면 `actor.SetCredits(...)`가 발생시키는 `PropertyChanged`를 그 창이 동기적으로 받아 `RefreshCreditsPanel` → `SyncCreditsFromManagedItems`를 즉시 실행한다 — 이 시점에 item이 아직 `_managedItems`에 남아있고 `item.Actors`도 그대로면 "아직 반영 안 된 항목"으로 오인해 방금 지운 품번을 곧바로 되살려버려, 결과적으로 완전삭제해도 Credits에서 전혀 지워지지 않는 버그가 있었다(실제 재현 확인). **`_managedItems.Remove(item)`을 먼저 호출하고 그 다음에 `OnFileDeleted`를 호출하도록 순서를 바꿔** 해결했다 — 재동기화 시점에 item이 더 이상 스캔 대상이 아니므로 되살아나지 않는다.

## "배우 동기화" 버튼

(`MainWindow.SyncActors_Click`, 2026-08-10 추가, 같은 날 "속성" 버튼 밑 줄로 위치 변경, 구현 완료): 관리 리스트 툴바 5번째 줄("속성"/"태그 관리"/"배우 관리"/"시리즈 관리" 줄 바로 밑), "시리즈 동기화" 버튼 왼쪽에 있다. **도구 메뉴의 동일 항목은 2026-09-13 삭제되었다**(사용자 요청 — 도구 메뉴에서 동기화 항목 2개 제거, 관리 리스트 툴바 버튼은 그대로 유지) — 이 버튼과 핸들러(`SyncActors_Click`)는 그대로 남아있고 메뉴 진입점만 없어졌다. "시리즈 동기화"와 같은 방향의 일괄 동기화("Credits → 관리 리스트")를 배우에 대해 수행한다 — 모든 배우의 Credits(품번)를 훑어서, 그 품번과 일치하지만 아직 그 배우가 `Actors`에 지정되어 있지 않은 관리 리스트 항목에 배우를 추가한다. **배우는 시리즈와 달리 여러 명을 동시에 지정할 수 있는 목록이라 "충돌"이라는 개념이 없다** — 기존에 지정된 다른 배우를 덮어쓰거나 지우지 않고 그냥 없는 것만 추가한다(따라서 결과 대화상자에는 업데이트 목록만 있고 시리즈 동기화와 달리 충돌 섹션이 없다). 결과 대화상자 문구 조립은 `MainWindow.BuildSyncResultMessage`를 시리즈 동기화와 공유한다(충돌 목록을 빈 리스트로 넘기면 자동으로 그 섹션이 생략됨).
...
`ActorManagerWindow` 맨 아래에 카드 형태의 상태바(둥근 모서리 `CornerRadius="6"`, 옅은 회색 배경 `#FAFAFA` + 옅은 테두리 `#E0E0E0`, 오른쪽 패널을 포함해 창 전체 폭을 가로지름)가 있다. **실제 사용자 결정이 필요 없는** 단순 안내/오류/성공 메시지는 대화상자(`MessageBox.Show`) 대신 이 상태바에 표시한다 — 예를 들어 "배우 이름을 입력하세요.", "이미 존재하는 배우입니다.", **"이미 추가된 품번입니다: '코드'"**(사용자가 명시적으로 지적한 케이스), "관리 리스트에 이 품번의 파일이 없습니다.", "썸네일을 만들 수 없습니다: {오류}" 등. 배우/썸네일/품번 추가·변경·삭제가 성공했을 때도 "'이름' 배우를 추가했습니다." 같은 메시지를 남긴다. 반대로 **실제 Yes/No 결정이 필요한 확인**(배우 삭제 확인, 품번을 Credits에서 제거할지 확인)은 그대로 `MessageBox.Show`(`MessageBoxButton.YesNo`)를 사용한다.

- **종류별 아이콘/색 구분**(2026-08-29 추가, 사용자 요청 — "적당히 이쁘게 꾸며줘", 페이드아웃 등 애니메이션은 명시적으로 제외): 성공(`StatusType.Success`) "✓" 초록(`#2E7D32`), 경고(`StatusType.Warning`, 입력 누락·이미 존재함·중복 품번·이미지 못 찾음 등) "⚠" 주황(`#B26A00`), 정보(`StatusType.Info`, "관리 리스트에 이 품번의 파일이 없습니다" 같은 단순 안내) "ℹ" 파랑(`#1565C0`), 오류(`StatusType.Error`, 썸네일 생성 실패 등 예외) "✕" 빨강(`#C62828`). `SetStatus(string message, StatusType type)` private 헬퍼가 아이콘(`StatusIcon`)과 텍스트(`StatusText`) 둘 다 갱신하며, 메시지는 다음 갱신 전까지 자동으로 사라지지 않는다.
- **품번 추가 성공 메시지에 "파일 확인" 여부를 덧붙인다**(2026-08-29 추가, [시리즈 관리](series-management.md)와 동일하게 맞춤, 사용자 요청) — `AddCreditToActor`가 방금 추가한 품번과 파일명(확장자 제외)이 일치하는 관리 리스트 항목이 있는지(`hasMatch`) 확인해, 하나라도 있으면 성공 메시지 끝에 `" (파일 확인)"`을 붙인다 — 예: `'ssni-123' 품번을 추가했습니다. (파일 확인)`. 일치하는 파일이 없으면(아직 라이브러리에 없는 품번을 미리 등록해두는 경우) 접미사 없이 `'{code}' 품번을 추가했습니다.`만 남는다. "작품 추가" 대화상자와 드래그 앤 드롭 둘 다 `AddCreditToActor`를 거치므로 두 경로 모두 적용된다.

[태그 관리](tag-management.md)/[시리즈 관리](series-management.md)도 동일한 패턴(구조·스타일·종류 구분 모두)을 공유한다.
