...
- **배우 관리 화면(`ActorManagerWindow`, 기본 크기 1000x1000 — 마지막으로 조절한 크기가 있으면 그 크기로 열림, 아래 "창 전체 크기도 기억한다" 참고)**: 메인창 "도구 > 배우 관리" 메뉴 또는 관리 리스트 툴바의 "배우 관리" 버튼(태그 관리 버튼 옆)으로 연다.
  ...
  - **창 전체 크기도 기억한다**(2026-08-27 추가, **구현 완료**, 사용자 요청) — 다른 주요 창들은 위치(Left/Top)만 `WindowPositionMemory`로 기억하고 크기는 항상 XAML 기본값(1000x1000)인데, `ActorManagerWindow`는 크기(Width/Height)도 함께 기억한다. `RememberedWindowWidth`/`RememberedWindowHeight` 정적 프로퍼티(위 `RememberedRightPanelWidth`와 같은 패턴)를 두고, 생성자에서 값이 있으면 `Width`/`Height`에 적용하고, `Closed` 시점에 `WindowState == Normal ? Width/Height : RestoreBounds.Width/Height`(최대화 상태로 닫았다면 최대화 이전 "정상" 크기 — `MainWindow` 자신의 크기 기억 로직과 동일한 원칙)를 저장한다. `MainWindow`가 시작 시 `AppSettings.ActorManagerWindowWidth`/`Height`에서 불러와 채우고 종료 시 다시 저장한다. 같은 세션 내 재오픈과 앱 완전 재시작(`settings.json` 왕복) 둘 다 확인함 → [공통 관리](common-management.md)의 "창 관리 정책" 규칙 4 참고.
