...
창 맨 아래에 얇은 상태바(`StatusText`, 회색 배경 `#F5F5F5` + 위쪽 테두리)가 있다. **실제 사용자 결정이 필요 없는** 단순 안내/오류/성공 메시지는 대화상자(`MessageBox.Show`) 대신 이 상태바에 표시한다 — 예를 들어 "태그 이름을 입력하세요.", "이미 존재하는 태그입니다.", **이미 추가된 품번입니다: '코드'"**(사용자가 명시적으로 지적한 케이스), "관리 리스트에 이 품번의 파일이 없습니다." 등. 태그/품번 추가·변경·삭제가 성공했을 때도 "'이름' 태그를 추가했습니다." 같은 메시지를 상태바에 남겨 결과를 바로 확인할 수 있게 했다. 반대로 **실제 Yes/No 결정이 필요한 확인**(태그 삭제 확인, 품번을 Credits에서 제거할지 확인)은 그대로 `MessageBox.Show`(`MessageBoxButton.YesNo`)를 사용한다 — 상태바로는 사용자의 결정을 받을 수 없기 때문이다. `SetStatus(string)` private 헬퍼가 `StatusText.Text`를 갱신한다. [배우 관리](actor-management.md)/[시리즈 관리](series-management.md)도 동일한 패턴을 공유한다.
