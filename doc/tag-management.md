...
- **테두리(2026-08-27 추가, 구현 완료, 사용자 요청)**: 품번 칩 목록을 감싼 `ScrollViewer`를 회색 `CornerRadius="8"` 둥근 `Border`(배경 없음)로 한 번 더 감쌌다. `ScrollViewer`에 `Padding="6"`도 함께 줘서 칩이 둥근 테두리 모서리에 바로 닿지 않게 여백을 뒀다. 처음엔 태그 관리 창에만 적용했는데, 같은 요청이 [배우 관리](actor-management.md)/[시리즈 관리](series-management.md)에도 이어져 세 창 모두 동일한 스타일이 됐다.
...
- **"적용" 버튼**(2026-08-31 추가, "추가" 버튼 왼쪽, 사용자 요청): 선택한 태그의 Credits 중 관리 리스트에 아직 매칭되는 항목이 없는(연한 색 칩) 품번을 일괄로 [동영상 파일 관리](video-file-management.md)의 "파일 없이 추가"와 동일한 신규(placeholder, `IsPlaceholder=true`/`IsValid=false`/`IsExist=false`) 항목으로 등록한다(`ApplyMissingCredits_Click`, [시리즈 관리](series-management.md)의 같은 이름 버튼과 동일한 패턴). 이미 매칭되는 항목이 있는 품번(진한 색 칩)은 건드리지 않는다. 새로 만든 항목은 이 태그가 붙어있음을 이미 알고 있으므로 `Tags`에도 이 태그를 채운다(태그는 다대다라 `SetTags(new List<string> { tag.Name })`로 시작). 결과는 상태바에 "N개 품번을 신규 파일로 등록했습니다."(등록할 대상이 없으면 정보 메시지)로 표시되며, 확인 대화상자 없이 바로 실행된다(기존 값을 덮어쓰지 않는 안전한 연산이므로). 이렇게 등록된 항목은 메인 창의 [빠른 보기 패널](video-file-management.md) "신규 파일" 체크박스로 모아 볼 수 있다.

## 태그 Credits ↔ 관리 리스트 상호 동기화 (`TagCreditSync.cs`, 2026-09-13 추가)

**배경**: 배우/시리즈에는 이미 [배우 관리](actor-management.md)의 `ActorCreditSync`/[시리즈 관리](series-management.md)의 `SeriesCreditSync`가 있어서 파일명 변경(품번 변경)·완전삭제 시 Credits가 자동으로 따라갔지만, 태그에는 대응하는 클래스가 아예 없었다. 그 결과 파일 이름을 바꾸거나 완전삭제해도 태그 Credits는 갱신되지 않고 예전 품번이 "파일 없음"(연한 색) 상태의 유령 참조로 계속 남는 버그가 있었다 — 실사용 데이터를 분석해보니 태그 Credits가 참조하는 품번 중 상당수가 현재 관리 리스트의 어떤 파일과도 매칭되지 않는 것으로 확인되어 발견했다. `ActorCreditSync`/`SeriesCreditSync`와 완전히 동일한 패턴으로 이 클래스를 추가해 해결했다(태그도 배우처럼 다대다 관계라 로직도 배우 쪽과 동일하다).

- `OnFileRenamed(item, oldFileName, masterTags)`: 파일명이 바뀌면(`RenameHelper.TryRenameManagedItemTo`/`TryEditFullPath` 성공 시 호출) ① 이 파일에 이미 지정된 태그들 중 옛 품번을 Credits로 갖고 있던 태그는 새 품번으로 값을 갱신하고, ② 새 품번이 이미 다른(아직 파일 없이 연한 색으로만 있던) 태그의 Credits와 일치하면 그 태그를 이 파일의 `Tags`에 자동으로 추가한다. `RenameHelper`가 `masterTags`를 새로 받도록 확장됐다(기존 `masterActors`/`masterSeries` 매개변수 앞에 추가) — `MainWindow`의 F2/우클릭 "이름변경"과 [속성 관리](properties-management.md)의 파일명/경로 수정 모두 같은 경로를 타므로 어느 진입점으로 rename해도 동일하게 동기화된다.
- `OnTagRemovedFromItem(item, removedTagName, masterTags)`: [속성 관리](properties-management.md)의 `PropertiesWindow`에서 태그 체크박스를 해제하고 커밋하면, 그 태그의 Credits에서 이 파일의 품번을 제거한다.
- `OnFileDeleted(item, masterTags)`: [동영상 파일 관리](video-file-management.md)의 "완전삭제"에서 호출한다. 이 파일에 태깅되어 있던 모든 태그에 대해 내부적으로 `OnTagRemovedFromItem`을 호출해서, 각 태그의 Credits에서 이 파일의 품번을 제거한다. **"제거"(소프트 삭제, 복구 가능)는 대상이 아니다.**
- **호출 순서 버그(2026-09-13 발견·수정, 실제 재현 확인)** — 처음 구현 시 `MainWindow.PermanentlyDeleteManaged_Click`에서 `_managedItems.Remove(item)`보다 **먼저** `OnFileDeleted`를 호출했는데, 태그 관리 창을 열어두고 마침 그 태그를 선택해둔 상태로 완전삭제하면 `tag.SetCredits(...)`가 발생시키는 `PropertyChanged`를 그 창이 동기적으로 받아 `RefreshCreditsPanel` → `SyncCreditsFromManagedItems`를 즉시 실행했다 — 이 시점에 item이 아직 `_managedItems`에 남아있고 `item.Tags`도 그대로면 "아직 반영 안 된 항목"으로 오인해 방금 지운 품번을 곧바로 되살려버려, 완전삭제해도 Credits에서 전혀 지워지지 않는 것처럼 보이는 버그가 있었다(태그 관리 창을 열어두고 그 태그가 선택된 채로 테스트하다가 직접 재현·발견함). **`_managedItems.Remove(item)`을 먼저 호출한 뒤에 `TagCreditSync`/`ActorCreditSync`/`SeriesCreditSync`의 `OnFileDeleted`를 모두 호출하도록 순서를 바꿔** 해결했다 — [배우 관리](actor-management.md)/[시리즈 관리](series-management.md)도 같은 호출 순서를 공유하므로 이 두 클래스에 잠재해 있던 같은 종류의 버그도 함께 해결됐다.

## "태그 동기화" 버튼

(`MainWindow.SyncTags_Click`, 2026-09-13 추가): 관리 리스트 툴바 5번째 줄("속성"/"태그 관리"/"배우 관리"/"시리즈 관리" 줄 바로 밑), "배우 동기화" 버튼 왼쪽에 있다(도구 메뉴에는 없음 — 2026-09-13에 배우/시리즈 동기화와 함께 메뉴에서 제거되고 툴바 버튼만 남았다). [배우 관리](actor-management.md)의 "배우 동기화"와 완전히 동일한 방향의 일괄 동기화("Credits → 관리 리스트")를 태그에 대해 수행한다 — 모든 태그의 Credits(품번)를 훑어서, 그 품번과 일치하지만 아직 그 태그가 `Tags`에 지정되어 있지 않은 관리 리스트 항목에 태그를 추가한다. 태그도 배우처럼 다대다라 "충돌" 개념이 없다 — 기존 태그 지정을 덮어쓰거나 지우지 않고 없는 것만 추가한다. `TagCreditSync`가 막는 것은 **앞으로**(rename/완전삭제 시점)의 드리프트뿐이라, 이 버튼은 그 클래스가 생기기 전부터 이미 쌓여 있던 기존 데이터의 어긋남(태그 Credits는 알고 있지만 실제 항목의 `Tags`에는 아직 반영 안 된 경우)을 한 번에 정리하는 용도다. 실제 데이터(태그 44개, 영상 약 3,600건)에 대해 실행한 결과 "동기화할 항목이 없습니다"로 나왔다 — 기존 `SyncCreditsFromManagedItems`(태그를 선택할 때마다 도는 개별 동기화)가 이미 이 방향은 잘 유지해왔다는 뜻이며, 실제 드리프트는 이 방향이 아니라 **삭제/rename 후 Credits에 남는 유령 품번** 쪽이었음을 확인해준다. 관리 리스트에 없는 품번(Credits에는 있지만 아직 파일이 없는 "알려진 품번")은 건드리지 않는다 — 정상적인 상태이므로 조용히 건너뛴다.

## 상태바 (2026-08-29 추가, 2026-08-29 스타일 개선, 구현 완료, 사용자 요청)

창 맨 아래에 카드 형태의 상태바(둥근 모서리 `CornerRadius="6"`, 옅은 회색 배경 `#FAFAFA` + 옅은 테두리 `#E0E0E0`)가 있다. **실제 사용자 결정이 필요 없는** 단순 안내/오류/성공 메시지는 대화상자(`MessageBox.Show`) 대신 이 상태바에 표시한다 — 예를 들어 "태그 이름을 입력하세요.", "이미 존재하는 태그입니다.", **"이미 추가된 품번입니다: '코드'"**(사용자가 명시적으로 지적한 케이스), "관리 리스트에 이 품번의 파일이 없습니다." 등. 태그/품번 추가·변경·삭제가 성공했을 때도 "'이름' 태그를 추가했습니다." 같은 메시지를 상태바에 남겨 결과를 바로 확인할 수 있게 했다. 반대로 **실제 Yes/No 결정이 필요한 확인**(태그 삭제 확인, 품번을 Credits에서 제거할지 확인)은 그대로 `MessageBox.Show`(`MessageBoxButton.YesNo`)를 사용한다 — 상태바로는 사용자의 결정을 받을 수 없기 때문이다.

- **종류별 아이콘/색 구분**(2026-08-29 추가, 사용자 요청 — "적당히 이쁘게 꾸며줘", 페이드아웃 등 애니메이션은 명시적으로 제외): 메시지 앞에 아이콘 하나가 붙고 아이콘과 글자색이 함께 종류에 맞춰 바뀐다 — 성공(`StatusType.Success`) "✓" 초록(`#2E7D32`), 경고(`StatusType.Warning`, 입력 누락·이미 존재함·중복 품번 등) "⚠" 주황(`#B26A00`), 정보(`StatusType.Info`, "관리 리스트에 이 품번의 파일이 없습니다" 같은 단순 안내) "ℹ" 파랑(`#1565C0`), 오류(`StatusType.Error`, 예외 발생) "✕" 빨강(`#C62828`). `SetStatus(string message, StatusType type)` private 헬퍼가 아이콘(`StatusIcon`)과 텍스트(`StatusText`) 둘 다 갱신한다. 메시지는 자동으로 사라지지 않고 다음 상태바 갱신 전까지 그대로 유지된다.
- **품번 추가 성공 메시지에 "파일 확인" 여부를 덧붙인다**(2026-08-29 추가, [시리즈 관리](series-management.md)와 동일하게 맞춤, 사용자 요청) — `AddCreditToTag`가 방금 추가한 품번과 파일명(확장자 제외)이 일치하는 관리 리스트 항목이 있는지(`hasMatch`) 확인해, 하나라도 있으면 성공 메시지 끝에 `" (파일 확인)"`을 붙인다 — 예: `'kr-123' 품번을 추가했습니다. (파일 확인)`. 일치하는 파일이 없으면(아직 라이브러리에 없는 품번을 미리 등록해두는 경우) 접미사 없이 `'{code}' 품번을 추가했습니다.`만 남는다. "추가" 대화상자와 드래그 앤 드롭 둘 다 `AddCreditToTag`를 거치므로 두 경로 모두 적용된다.

[배우 관리](actor-management.md)/[시리즈 관리](series-management.md)도 동일한 패턴(구조·스타일·종류 구분 모두)을 공유한다.
