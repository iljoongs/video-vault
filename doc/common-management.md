...
**관련 파일**: `App.xaml`/`.xaml.cs`, `ImageLoadHelper.cs`, `WindowsIconHelper.cs`, `ThumbnailPathConverter.cs`, `ThumbnailHelper.cs`, `OriginalImageWindow.xaml`/`.xaml.cs`, `DragDropImageHelper.cs`, `AppPaths.cs`, `AppSettings.cs`, `SettingsRepository.cs`, `SingleInstanceWindow.cs`, `WindowPositionMemory.cs`, `WindowSizeMemory.cs`, `WindowSnapHelper.cs`, `FormatUtil.cs`
...
- **주요 창의 마지막 화면 위치/크기** (`AppSettings.WindowPositions`/`WindowSizes`, 위치는 2026-08-04 추가, 크기는 2026-08-27~29 추가, **구현 완료**) — 아래 "창 관리 정책"의 규칙 4 참고. `WindowPositionMemory`/`WindowSizeMemory`가 각각 메모리에서 관리하며, 이 설정 필드들은 그 스냅샷을 불러오고(`ApplySettings`) 저장하는(`SaveSettings`) 용도다.
...
- **규칙 4: 마지막으로 열려 있던 화면 위치를 기억한다**(2026-08-04 추가, **구현 완료**). 창을 닫을 때의 위치(Left/Top)를 창 종류별로 기억해뒀다가, 다음에 그 종류의 창을 열면 같은 위치에 연다. 프로그램을 재시작해도 유지된다(설정 파일에 저장). **`PropertiesWindow`/`TagManagerWindow`/`ActorManagerWindow`/`SeriesManagerWindow` 4개는 크기(Width/Height)까지 함께 기억한다**(2026-08-27 `ActorManagerWindow`에 먼저 추가, 2026-08-29 나머지 3개로 확장, 사용자 요청) — `WindowPositionMemory`는 5개 주요 창 전부의 위치만 다루므로, 크기는 별도의 공용 저장소 `WindowSizeMemory.cs`(같은 패턴: 창 클래스 이름을 키로 Width/Height를 기억, `AppSettings.WindowSizes`에 저장)를 이 4개 창이 생성자/`Closed`에서 직접 호출해 저장·복원한다. `FolderListWindow`만 크기 기억 대상에서 제외된다(폴더 목록 창은 요청 범위 밖) — 위치만 기억하고 크기는 XAML 기본값으로 매번 연다.
...
- **규칙 2(동시 갱신)의 실제 구현**:
  - `MainWindow._managedView`(관리 리스트의 `ICollectionView`)에 **라이브 필터링/정렬**(`ICollectionViewLiveShaping.IsLiveFiltering`/`IsLiveSorting` + `LiveFilteringProperties`/`LiveSortingProperties`, `EnableLiveShaping()`)을 켜뒀다. 필터/정렬에 쓰이는 속성(`IsValid`/`IsExist`/`HasThumbnail`/`FileName`/`FolderName`/`Tags`/`Actors`/`Series`, 정렬 가능한 모든 컬럼 속성)을 감시 대상으로 등록해두면, 그 속성이 바뀔 때마다(어느 창에서 바꿨든) WPF가 자동으로 그 항목의 필터 통과 여부·정렬 위치를 재평가한다 — `_managedView.Refresh()`를 수동으로 호출할 필요가 없다(다만 창을 닫을 때는 여전히 안전망으로 `Refresh()`도 함께 호출한다). **`IsExist`/`HasThumbnail` 누락 버그(2026-08-31 수정)** — [동영상 파일 관리](video-file-management.md)의 "빠른 보기 패널"에 이 두 속성 기준 체크박스("신규 파일"/"썸네일 있음"/"썸네일 없음")를 추가하면서, 정작 이 등록 목록에는 `IsValid`만 있고 `IsExist`/`HasThumbnail`은 빠져 있었다 — 그 결과 앱을 껐다 켜지 않고 실행 중에 이 두 속성이 바뀌는 경우(파일 재사용으로 `IsExist`가 `true`가 될 때, 파일을 못 찾아 `IsExist=false`가 될 때, 썸네일 추가/삭제로 `HasThumbnail`이 바뀔 때) 체크박스 조건과 실제 화면 표시가 어긋나는 버그가 있었다. 두 속성을 목록에 추가해 바로잡았다.
