...
- **검색(필터) — 리스트 위 상시 UI**: 관리 리스트 위쪽(툴바 5~6번째 줄, 2026-08-16부터 두 줄로 분리)에 필터 영역이 항상 표시된다 (더 이상 헤더를 우클릭해서 열지 않는다).
  ...
  - **빠른 보기 패널**(2026-08-31 추가, 2026-08-31 재설계 — "제거된 항목도 표시"/"모든 파일" 체크박스를 없애고 이 패널의 체크박스로 통합, 2026-08-31 "일반 파일" 추가, **구현 완료**, 사용자 요청): 썸네일 뷰어 **왼쪽**에 둥근 모서리(`CornerRadius="8"`) 테두리로 감싼 별도 패널이 있다. 체크박스 3개("일반 파일"/"삭제 파일"/"신규 파일", 이 순서) + 구분선 + "아이콘만 보기" 체크박스(아래 항목 참고, 2026-08-31 이 패널로 위치 이동)로 구성된다. **세 체크박스는 서로 독립적인 OR 토글이다** — `MainWindow.FilterManagedItem`에서 `(일반 파일 && item.IsValid) || (삭제 파일 && !item.IsValid) || (신규 파일 && !item.IsExist)`로 판정한다(이름/폴더명/태그/배우/시리즈 필터는 그 위에 그대로 AND로 추가 적용됨). **"일반 파일"**(기본값 켜짐)은 정상 상태(`IsValid = true`)인 항목을 보여준다 — 꺼지면 이 항목들이 목록에서 숨겨진다(처음 구현 시에는 이 항목들이 항상 무조건 보였으나, "삭제 파일"/"신규 파일"만 켜서 그 항목들만 보고 싶다는 요청으로 이 체크박스가 추가되며 토글 가능해짐). **"삭제 파일"**(기본값 꺼짐)을 켜면 제거된(`IsValid = false`) 항목도 함께 보이며(예전 "제거된 항목도 표시" 체크박스와 동일한 효과, 그 체크박스는 삭제되고 이 체크박스가 그 역할을 대신함), **"신규 파일"**(기본값 꺼짐)을 켜면 아직 실제 파일을 구하지 못한(`IsExist = false`) 항목도 함께 보인다("파일 없이 추가"로 만든 placeholder 포함, [관리 리스트]의 `IsExist`/`IsPlaceholder` 설명 참고 — 이런 항목은 보통 `IsValid`도 함께 `false`라 "삭제 파일"을 켜지 않는 한 이 체크박스가 유일한 노출 경로다). `IsValid = true`이면서 `IsExist = false`인 항목(파일을 못 찾는 활성 항목)은 "일반 파일"이 켜져 있으면(기본값) 계속 보이며, `MultiDataTrigger`로 주황빛(`OrangeRed`)으로 표시된다(2026-08-06 추가). 제거된 상태로 보이는 항목은 **글자 색이 회색**으로 표시되어 구분된다(`ItemContainerStyle`의 `DataTrigger Binding="{Binding IsValid}"`). 세 체크박스 모두 켜져 있으면 결과적으로 전체 항목이 보인다(예전 "모든 파일" 체크박스와 동일한 결과를 조합으로 얻을 수 있음). 선택 상태는 `AppSettings.QuickFilterNormalFiles`/`QuickFilterDeletedFiles`/`QuickFilterNewFiles`에 저장되어 다음 실행에도 유지된다.
