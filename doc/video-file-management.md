...
- **검색(필터) — 리스트 위 상시 UI**: 관리 리스트 위쪽(툴바 5~6번째 줄, 2026-08-16부터 두 줄로 분리)에 필터 영역이 항상 표시된다 (더 이상 헤더를 우클릭해서 열지 않는다).
  ...
  - **빠른 보기 패널**(2026-08-31 추가, 2026-08-31 재설계 — "제거된 항목도 표시"/"모든 파일" 체크박스를 없애고 이 패널의 두 체크박스로 통합, **구현 완료**, 사용자 요청): 썸네일 뷰어 **왼쪽**에 둥근 모서리(`CornerRadius="8"`) 테두리로 감싼 별도 패널이 있다. 체크박스 2개("삭제 파일"/"신규 파일") + 구분선 + "아이콘만 보기" 체크박스(아래 항목 참고, 2026-08-31 이 패널로 위치 이동)로 구성된다. **기본 목록(`item.IsValid`인 항목)은 항상 그대로 보이고, 두 체크박스는 그 위에 항목을 추가로 더 보여주는 additive 토글이다**(처음 구현 시에는 "삭제 파일"이 제거된 항목만 걸러 보여주는 전용 보기였으나, 기존 활성 목록을 가리는 게 불편하다는 피드백으로 당일 재설계됨) — `MainWindow.FilterManagedItem`에서 `item.IsValid || (삭제 파일 && !item.IsValid) || (신규 파일 && !item.IsExist)`로 판정한다(이름/폴더명/태그/배우/시리즈 필터는 그 위에 그대로 AND로 추가 적용됨). **"삭제 파일"**을 켜면 제거된(`IsValid = false`) 항목도 기존 목록에 함께 보이며(예전 "제거된 항목도 표시" 체크박스와 동일한 효과, 그 체크박스는 삭제되고 이 체크박스가 그 역할을 대신함), **"신규 파일"**을 켜면 아직 실제 파일을 구하지 못한(`IsExist = false`) 항목도 함께 보인다("파일 없이 추가"로 만든 placeholder 포함, [관리 리스트]의 `IsExist`/`IsPlaceholder` 설명 참고 — 이런 항목은 보통 `IsValid`도 함께 `false`라 "삭제 파일"을 켜지 않는 한 이 체크박스가 유일한 노출 경로다). `IsValid = true`이면서 `IsExist = false`인 항목(파일을 못 찾는 활성 항목)은 두 체크박스 상태와 무관하게 원래부터 항상 보이며, `MultiDataTrigger`로 주황빛(`OrangeRed`)으로 표시된다(2026-08-06 추가). 제거된 상태로 보이는 항목은 **글자 색이 회색**으로 표시되어 구분된다(`ItemContainerStyle`의 `DataTrigger Binding="{Binding IsValid}"`). 선택 상태는 `AppSettings.QuickFilterDeletedFiles`/`QuickFilterNewFiles`에 저장되어 다음 실행에도 유지되며, 둘 다 기본값은 꺼짐이다.
