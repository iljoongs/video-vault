...
- **검색(필터) — 리스트 위 상시 UI**: 관리 리스트 위쪽(툴바 5~6번째 줄, 2026-08-16부터 두 줄로 분리)에 필터 영역이 항상 표시된다 (더 이상 헤더를 우클릭해서 열지 않는다).
  ...
  - "필터 초기화" 버튼으로 파일명/폴더명 검색어, 시리즈 선택, 태그 선택, 배우 선택을 한 번에 초기화한다 (아래 빠른 보기 패널의 체크박스들은 초기화 대상이 아니다).
  - **빠른 보기 패널**(2026-08-31 추가, 2026-08-31 재설계 — "제거된 항목도 표시"/"모든 파일" 체크박스를 없애고 이 패널의 체크박스로 통합, 2026-08-31 "일반 파일"/"썸네일 파일" 추가, 2026-08-31 "썸네일 파일"→"썸네일 있음"으로 이름 변경 + "썸네일 없음" 추가, 2026-08-31 두 축(상태/썸네일) AND 결합으로 재수정, **구현 완료**, 사용자 요청): 썸네일 뷰어 **왼쪽**에 둥근 모서리(`CornerRadius="8"`) 테두리로 감싼 별도 패널이 있다. 체크박스 5개("일반 파일"/"삭제 파일"/"신규 파일"/"썸네일 있음"/"썸네일 없음", 이 순서) + 구분선 + "아이콘만 보기" 체크박스(아래 항목 참고, 2026-08-31 이 패널로 위치 이동)로 구성된다. **다섯 체크박스는 두 축으로 나뉜다** — **상태 축**("일반 파일"/"삭제 파일"/"신규 파일")과 **썸네일 축**("썸네일 있음"/"썸네일 없음"). **같은 축 안에서는 OR**(예: "삭제 파일"+"신규 파일" 동시 체크 = 제거됨 ∪ 신규), **두 축 사이는 AND**로 좁혀진다(예: "신규 파일"+"썸네일 있음" 동시 체크 = 신규 ∩ 썸네일 있음인 항목만). 어느 한 축을 아예 체크하지 않았으면 그 축은 조건 없음(모두 통과)으로 취급하되, 두 축 다 아무것도 안 켜져 있으면 전체가 숨겨진다. `MainWindow.FilterManagedItem`에서 `statusMatch`(상태 축, 비활성 시 `true`)와 `thumbnailMatch`(썸네일 축, 비활성 시 `true`)를 각각 구해 `statusMatch && thumbnailMatch`로 판정한다(이름/폴더명/태그/배우/시리즈 필터는 그 위에 그대로 추가 AND 적용됨). **"일반 파일"**(기본값 켜짐)은 정상 상태(`IsValid = true`)인 항목을 보여준다 — 꺼지고 다른 상태 체크박스도 없으면 상태 축이 비활성화된다(처음 구현 시에는 이 항목들이 항상 무조건 보였으나, "삭제 파일"/"신규 파일"만 켜서 그 항목들만 보고 싶다는 요청으로 이 체크박스가 추가되며 토글 가능해짐). **"삭제 파일"**(기본값 꺼짐)을 켜면 제거된(`IsValid = false`) 항목도 상태 축에 포함되며(예전 "제거된 항목도 표시" 체크박스와 동일한 효과, 그 체크박스는 삭제되고 이 체크박스가 그 역할을 대신함), **"신규 파일"**(기본값 꺼짐)을 켜면 아직 실제 파일을 구하지 못한(`IsExist = false`) 항목도 상태 축에 포함된다("파일 없이 추가"로 만든 placeholder 포함, [관리 리스트]의 `IsExist`/`IsPlaceholder` 설명 참고 — 이런 항목은 보통 `IsValid`도 함께 `false`라 "삭제 파일"을 켜지 않는 한 이 체크박스가 유일한 노출 경로다). **"썸네일 있음"**(기본값 꺼짐, 2026-08-31 이전에는 "썸네일 파일"이라는 이름이었다)을 켜면 썸네일이 지정된(`HasThumbnail = true`) 항목만 썸네일 축을 통과하고, **"썸네일 없음"**(2026-08-31 추가, 기본값 꺼짐)을 켜면 썸네일이 없는(`HasThumbnail = false`) 항목만 통과한다 — **상태 축과 무관하게 독립적으로 동작하다가, 상태 축의 체크박스가 하나라도 함께 켜지면 그 안에서 다시 걸러낸다**(예: "썸네일 있음"만 켜면 상태 무관하게 썸네일 있는 항목 전부, "신규 파일"+"썸네일 있음"을 함께 켜면 그중 신규 파일만). `IsValid = true`이면서 `IsExist = false`인 항목(파일을 못 찾는 활성 항목)은 "일반 파일"이 켜져 있으면(기본값) 계속 보이며, `MultiDataTrigger`로 주황빛(`OrangeRed`)으로 표시된다(2026-08-06 추가). 제거된 상태로 보이는 항목은 **글자 색이 회색**으로 표시되어 구분된다(`ItemContainerStyle`의 `DataTrigger Binding="{Binding IsValid}"`). 선택 상태는 `AppSettings.QuickFilterNormalFiles`/`QuickFilterDeletedFiles`/`QuickFilterNewFiles`/`QuickFilterThumbnailFiles`/`QuickFilterNoThumbnailFiles`에 저장되어 다음 실행에도 유지된다.
    - **버그 수정: 상태 축과 썸네일 축이 OR로 뒤섞여 있던 문제**(2026-08-31, **구현 완료**, 사용자 실사용 중 발견) — 처음 구현 때는 다섯 체크박스가 전부 하나의 OR로 묶여 있어서, 예를 들어 "신규 파일"과 "썸네일 있음"을 동시에 켜면 "신규 파일 중 썸네일 있는 것"이 아니라 "신규 파일 전체 ∪ 썸네일 있는 항목 전체"(합집합)가 보였다 — 사용자가 정확히 이 조합으로 재현해 보고함. 위에서 설명한 두 축 AND 방식으로 고쳐, 상태 축 체크박스와 썸네일 축 체크박스를 함께 켜면 교집합만 남도록 바로잡았다.
    - **"현재 표시" 개수**(2026-08-31 추가, "일반 파일" 체크박스 바로 위, 사용자 요청): `VisibleFileCountText`가 빠른 보기 체크박스 조합 + 파일명/폴더명 검색어 + 태그/배우 필터 + 시리즈 선택까지 **전부 적용한 뒤 실제로 목록에 남는 항목 수**를 "현재 표시: N개"로 보여준다. 맨 위 1번째 줄의 `ManagedCountText`("전체 N / 썸네일 N / 제거됨 N")는 필터와 무관한 라이브러리 전체 통계인 반면, 이 값은 지금 화면에 보이는 행 수 그 자체다. **구현 방식**: `_managedView`(`ICollectionView`, 런타임에는 `ListCollectionView`)가 구현하는 `INotifyCollectionChanged`를 구독해 `CollectionChanged`가 발생할 때마다 `_managedView.Cast<object>().Count()`로 다시 센다 — 체크박스 변경(`Refresh()` → `Reset`)뿐 아니라 검색어 입력, 라이브 필터링이 개별 항목을 넣고 빼는 경우(아래 "빠른 보기 체크박스와 실제 표시가 어긋나던 버그" 참고)까지 이벤트 하나로 전부 커버되어, 각 필터 변경 핸들러마다 별도로 갱신 호출을 추가할 필요가 없다. 시작 시 로딩 중(`_suppressAutoSave`)에는 항목이 하나씩 추가될 때마다 다시 세는 것을 건너뛰고(`ManagedCountText`와 같은 최적화 원칙), 로딩이 끝난 뒤 생성자에서 한 번만 계산한다.
    - **빠른 보기 체크박스와 실제 표시가 어긋나던 버그 수정**(2026-08-31, **구현 완료**) — "신규 파일"/"썸네일 있음"/"썸네일 없음" 체크박스를 추가하면서, 그 판정에 쓰는 `IsExist`/`HasThumbnail`을 `EnableLiveShaping()`의 `LiveFilteringProperties`에 등록하는 걸 빠뜨렸다(`IsValid`만 있었음 — "제거된 항목도 표시" 시절부터 있던 항목이라 그것만 등록돼 있었다). 그 결과 앱을 껐다 켜지 않고 실행 중에 `IsExist`/`HasThumbnail`이 바뀌는 경우(예: 같은 파일명으로 "재사용"하며 `IsExist`가 `true`로 바뀔 때, 시작 시 파일 존재 재확인, 속성 창에서 파일을 못 찾아 `IsExist=false`로 바뀔 때, 썸네일을 추가/삭제해 `HasThumbnail`이 바뀔 때 — 모두 항목이 이미 목록에 떠 있는 상태에서 값만 바뀌는 경우) 체크박스 조건상 더 이상 보이면 안 되는(또는 새로 보여야 하는) 항목이 `_managedView.Refresh()`를 수동으로 호출하기 전까지 화면에 그대로 남아있는(또는 나타나지 않는) 버그가 있었다. `LiveFilteringProperties`에 `IsExist`/`HasThumbnail`을 추가해, `IsValid`와 마찬가지로 이 두 속성이 바뀔 때마다 WPF가 자동으로 필터를 다시 평가해 항목을 즉시 넣고 빼도록 고쳤다.
