...
public partial class SeriesManagerWindow : Window
{
    ...
    /// <summary>"적용" 버튼(2026-08-31 추가, 사용자 요청) — 선택한 시리즈의 Credits 중 관리 리스트에 아직
    /// 매칭되는 항목이 없는(연한 색 칩) 품번을 일괄로 "파일 없이 추가"와 동일한 신규(placeholder) 항목으로
    /// 등록한다(`MainWindow.AddPlaceholderItem_Click`과 동일한 필드 조합 — `IsPlaceholder`/`IsValid`/`IsExist`).
    /// 이미 매칭되는 항목이 있는 품번(진한 색 칩)은 건드리지 않는다. 새로 만든 항목은 이 시리즈에 속함을 이미
    /// 알고 있으므로 `Series`도 함께 채워, 나중에 실제 파일이 생겨 재사용될 때 시리즈 지정을 다시 할 필요가
    /// 없게 한다. 기존 값을 덮어쓰지 않는 안전한 연산이라 확인 대화상자는 없다("배우 동기화"/"시리즈 동기화"와
    /// 같은 원칙).</summary>
    private void ApplyMissingCredits_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (series is null)
        {
            SetStatus("적용할 시리즈를 먼저 선택하세요.", StatusType.Warning);
            return;
        }
        ...
        var missingCodes = series.Credits.Where(code => !libraryCodes.Contains(code)).ToList();
        ...
        {
            _managedItems.Add(new ManagedVideoItem
            {
                FileName = code,
                FullPath = code,
    ...
    private void AddCreditToSeries(SeriesItem series, string code)
    {
        ...
        var matches = _managedItems.Where(m =>
            string.Equals(Path.GetFileNameWithoutExtension(m.FileName), code, StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        RefreshCreditsPanel();
        var fileConfirmedSuffix = matches.Count > 0 ? " (파일 확인)" : string.Empty;
